<template>
  <th class="col-header" :class="{ 'sort-column': isSortField }" :name="fieldName"
    :style="{ width: width - 14 + 'px' }">
    <div class="wrapper">
      <span class="text">{{ headerText }}</span>
      <span class="sort-icons" v-if="sortable">
        <svg v-if="!isSortAsc" @click="_sort('asc')" class="sort-asc" :fill="caretColor" v-html="caretUp24" width="16"
          height="16" :class="{ 'show-single': isSortDesc }"></svg>
        <svg v-if="!isSortDesc" @click="_sort('desc')" class="sort-desc" :fill="caretColor" v-html="caretDown24"
          width="16" height="16" :class="{ 'show-single': isSortAsc }"></svg>
      </span>
    </div>
  </th>
</template>
