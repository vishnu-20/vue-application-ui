<template>
    <div class="date-picker" v-show="isOpen">
      <div class="header">
        ...
        <span @click="switchView">{{ label }}</span>
  ...
  export default {
    ...
    computed: {
      label() {
        ...
        return `${this.current.year - 5} - ${this.current.year + 6}`;
      },
      calendarItems() {
    ...
    methods: {
      ...
      populateYears() {
        const startYear = this.current.year - 5;
      ...
      switchView() {
        if (this.currentView === 'DAY_VIEW') this.currentView = 'MONTH_VIEW';
        else if (this.currentView === 'MONTH_VIEW') this.currentView = 'YEAR_VIEW';
      },
      goPrevious() {
